...
const findClosestSun = (startCoords: GeolibCoordinates, bearing: number) => {
  ...
  const findClosestSunInner = (
    startCoords: GeolibCoordinates,
    bearing: number,
  ) => {
  ...
  const main = () => {
    ...
    findClosestSunInner(startCoords, bearing);
  };

  // main();

  // This entire outer function is because otherwise that 
  // global array would return other people's results
  return new Promise<findClosestSunType>(function (resolve, reject) {
    resolve(findClosestSunInner(startCoords, bearing));
  });
};
